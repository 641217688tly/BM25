...
class Estimator:
    ...
    def ndcg_at_n(self, retrieved, relevant, n=15):
        ...
        for query_id in retrieved:
            rel_ret_docs = self.retrieved_relevant(retrieved, relevant, query_id)
            for i in range(len(rel_ret_docs)):  # Calculate Discounted Cumulated Gain
                if i == 0:
            ...
            for j in range(len(temp_rel_ret_docs)):  # Calculate Ideal DCG vector and Normalized Discounted Cumulated Gain
                if j == 0:
...
def main():
    ...
    parser.add_argument('-p', '--path', required=True, help='Path to the large corpus.')
    ...
    if not os.path.exists(results_file_path):
        print("Results file not found. You should run the query_large_corpus.py script under large_corpus_handler folder.")
